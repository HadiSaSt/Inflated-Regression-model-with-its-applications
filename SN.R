...
(exp(par[1]+par[2]*x1+par[3]*x2)+1)
  etww=1-w0
  d <- (w0 * ((y>8)&(y<9)) + etww * dsn(y, xi=par[4], omega=par[5], alpha=par[6], log=FALSE))            
  -sum(log(d))
}
 c_i=c(0)
 u_i=rbind(c(0,0,0,0,1,0))
 init=c(.01,.01,.01,10,1,1)
...
pred = function(x1,x2){
  ...
  ran=sample(c(0,1), size=1, prob=p)
    if(ran==0) out[i]=med
    else if(ran==1) out[i]= rsn(1, xi=par[4], omega=par[5], alpha=par[6])            
