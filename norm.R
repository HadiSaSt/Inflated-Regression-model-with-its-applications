...
(exp(par[1]+par[2]*x1+par[3]*x2)+1)
  etww=1-w0
  d <- (w0 * ((y>8)&(y<9)) + etww * dnorm(y,par[4],par[5]))            
  -sum(log(d))
}
mi = min(y)
ma = max(y)
 c_i=c(0)
 u_i=rbind(c(0,0,0,0,1))
 init=c(.01,.01,.01,mean(y),sqrt(var(y)))
 out=constrOptim(init, lf_norm, NULL, ui=u_i, ci=c_i)
#out$value
#l=-sum(log(dnorm(y,1/mean(y),sqrt(var(y)))))
#2*7+2*out$value
#2*2+2*l
...
pred = function(x1,x2){
  ...
  ran=sample(c(0,1), size=1, prob=p)
    if(ran==0) out[i]=med
    else if(ran==1) out[i]= rnorm(1,par[4],par[5])
 }
   mean(out)
}
...
m1 = table((round(predd,1))==(round(y_te,1)))
n = length(y_te)
pre_model[iii] = m1[2]/n
te = data.frame(x1=x1_te,x2=x2_te)
m2 = table((round(fit,1))==(round(y_te,1)))
pre_lm[iii] = m2[2]/n
k1=4
k2=3
l1 = out$value
