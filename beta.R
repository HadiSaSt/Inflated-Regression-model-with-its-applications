...
(exp(par[1]+par[2]*x1+par[3]*x2)+1)
  etww=1-w0
  d <- (w0 * ((y>.8)&(y<1)) + etww * dbeta(y,par[4],par[5]))            
  -sum(log(d))
}
 c_i=c(0,0)
 u_i=rbind(c(0,0,0,1,0),c(0,0,0,0,1))
 init=c(.001,.001,.001,1,.1)
 
out=constrOptim(init, lf_norm, NULL, ui=u_i, ci=c_i)
#out$value
#l=-sum(log(dnorm(y,mean(y),sqrt(var(y)))))
#2*7+2*out$value
#2*2+2*l

f_norm=function(y,x1,x2){
  par = out$par
  w0 = (exp(par[1]+par[2]*x1+par[3]*x2))/
(exp(par[1]+par[2]*x1+par[3]*x2)+1)
  etww=1-w0
  d <- (w0 * par[6] + etww * dbeta(y,par[4],par[5]))            
  d
}
...
pred = function(x1,x2){
  ...
  ran=sample(c(0,1), size=1, prob=p)
    if(ran==0) out[i]=med
    else if(ran==1) out[i]= rbeta(1,par[4],par[5])
 }
   mean(out)
}
